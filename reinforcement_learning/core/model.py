...
class C6TransNet(nn.Module):
    ...
    def forward_transformer(self, x, return_attn=False):
        # x: (B, 361, C)
        attentions = []
        for block in self.blocks:
            ...
            if return_attn:
                x, attn = block['attn'](x, return_attn=True)
    ...
    def forward(self, x, move1_idx=None, return_attn=False):
        """
        Args:
            x: (B, 17, 19, 19)
        ...
        Returns:
            ...
            value: (B, 1)
            (Optional) attentions: List of (B, H, N, N)
        """
        B = x.size(0)
        ...
        if return_attn:
            features, attentions = self.forward_transformer(tokens, return_attn=True)
        else:
            features = self.forward_transformer(tokens)
        ...
        if return_attn:
            return policy1_logits, policy2_logits, value, attentions
